...
module OBS
  module Connection
    ...
    attr_reader :password, :driver, :socket

    def connect(uri = nil, password = nil)
      if uri
        @uri = URI.parse(uri)
        @password = password
      else
    ...
    module_function :connect, :config_from_toml, :password, :driver, :socket
  end
  ...
  module WS
    ...
    class Client
      ...
      def initialize(websocket, executor: :io, tracer: nil)
        ...
        @password = OBS::Connection.password
      ...
      def start_driver
        @driver = OBS::Connection.driver
        @socket = OBS::Connection.socket
        @driver.start
      ...
      def subscriptions=(value)
        ...
        reidentify if @state == :pending_identified || @state == :identified
      end
      ...
      def identify(msg)
        if auth_params = msg["authentication"]
          if password =
               @password.respond_to?(:call) ? @password.call : @password
            salt = auth_params["salt"]
            challenge = auth_params["challenge"]

            token =
              auth_token(password: password, salt: salt, challenge: challenge)
          end
        end

        send_message(
          Protocol::Messages::IdentifyMessage.new(
            {
      ...
      def reidentify
        send_message(
          Protocol::Messages::ReidentifyMessage.new(
            { "eventSubscriptions" => @subscriptions }.compact
          )
        )
      end
      ...
      def send_request(request)
        req_id, future =
          @response_dispatcher.register(default_executor, request)
        send_message(
          Protocol::Messages::RequestMessage.new(
            {
      ...
      def handle_message(payload)
        ...
        case @state
        when :pending_hello
          unless Protocol::Messages::HelloMessage === msg
            fail "Pending Hello, but received #{msg}"
          end
        when :pending_identified
          unless Protocol::Messages::IdentifiedMessage === msg
            fail "Pending Identified, but received #{msg}"
          end
          if Protocol::Messages::HelloMessage === msg
            fail "Duplicate Hello received #{msg}"
          end
        when :identified
          if Protocol::Messages::HelloMessage === msg
            fail "Duplicate Hello received #{msg}"
        ...
        when Protocol::Messages::IdentifiedMessage
          ...
          unless RPC_VERSIONS.include?(@negotiated_rpc_version)
            fail "RPC version not supported (#{@negotiated_rpc_version})"
          end
          @state = :identified
          @on_open.resolve
      ...
      class ResponseDispatcher
        ...
        def register(executor, request)
          ...
          @ongoing_requests[request_id] = { request: request, future: future }
    ...
    module Protocol
      ...
      module TypeConverters
        Any =
          TypeConverter.new(name: "Any") do
            def as_ruby(v)
              if v.nil?
                raise ConversionError.new("nil is not allowed", value: v)
              end
              v
            end
        ...
        Boolean =
          TypeConverter.new(name: "Boolean") do
            def as_ruby(b)
              unless true == b || false == b
                raise ConversionError.new("Boolean value is expected", value: b)
              end
              b
            end
            ...
            def as_json(b)
              if b.nil?
                raise ConversionError.new("nil is not allowed", value: b)
              end
              !!b
            end
          end
        ...
        Number =
          TypeConverter.new(name: "Number") do
            def as_ruby(f)
              unless Integer === f || Float === f
                raise ConversionError.new(
                        "Integer or Float value is expected",
                        value: f
                      )
              end
              f
        ...
        String =
          TypeConverter.new(name: "String") do
            def as_ruby(s)
              unless ::String === s
                raise ConversionError.new("String value is expected", value: s)
              end
              s.to_s
            end
            ...
            def as_json(s)
              if s.nil?
                raise ConversionError.new("nil is not allowed", value: s)
              end
              s.to_s
            end
          end
        ...
        Optional =
          Class
            .new do
              def [](element_type)
                TypeConverter.new(
                  name: "Optional[#{element_type.name}]",
        ...
        Array =
          Class
            .new do
              def [](element_type)
                TypeConverter.new(
                  name: "Array[#{element_type.name}]",
                  element_type: element_type
                ) do
                  def as_ruby(a)
                    unless ::Array === a
                      raise ConversionError.new(
                              "Array value is expected",
                              value: a
                            )
                    end
                  ...
                  def as_json(a)
                    if a.nil?
                      raise ConversionError.new("nil is not allowed", value: a)
                    end
        ...
        Object =
          TypeConverter.new(name: "Object") do
            def as_ruby(o)
              unless Hash === o
                raise ConversionError.new("Hash value is expected", value: a)
              end
              o.to_h
            end
            ...
            def as_json(o)
              if o.nil?
                raise ConversionError.new("nil is not allowed", value: o)
              end
              o.to_h
            end
            ...
            def [](fields)
              TypeConverter.new(
                name: "Object[#{fields.keys.map(&:to_s).join(", ")}]",
                fields: fields
              ) do
                def as_ruby(o)
                  unless Hash === o
                    raise ConversionError.new(
                            "Hash value is expected",
                            value: a
                          )
                  end
      ...
      class ServerMessage < Message
        def self.create(json)
          op = json.fetch("op")
          cls = CLASSES_BY_OPCODE[op] or
            fail "Unknown message type (op=#{op.inspect})"
